...
class MapCCFCalculations {
    ...
    addToImageOutput(imgArray){
        ...
        canvasLink.height = size;
        canvasLink.width = size * 2;
        canvasLink.style.width = size + "px";
        canvasLink.style.height = size * 2 + "px";
        for (let y = 0; y < canvasLink.height; y++){
            for (let x = 0; x < canvasLink.width; x++){
                let imgX = self.WIDTH / (canvasLink.width + 1);
                let imgY = self.HEIGHT / (canvasLink.height + 1);
                ...
                canvasLink.getContext("2d").putImageData(datum, x, y);
            }
        }
