...
function init() {
  document
    .querySelector("body")
    .addEventListener("game load done", mapCalculations);

  progressText = document.querySelector(".progressText");
  progressText.innerText = "Loading...";
  PromptMissingInfoContainer = document.querySelector(
    ".promptMissingInfoContainer",
  );
  PromptedMissingInfoCanvas = document.querySelector(
    ".promptMissingInfoContainer canvas",
  );
  PromptedMissingInfoCanvas.width = WIDTH;
  PromptedMissingInfoCanvas.height = HEIGHT;
  PromptLabel = document.querySelector(
    ".promptMissingInfoContainer label",
  );
  PromptField = document.querySelector(
  ...
  autoGeneratedCffTextField = document.querySelector(
    ".autoGeneratedCffTextField",
  );
  ImageOutputContainer = document.querySelector(
...
async function mapCalculations() {
  ...
  climateColorProperties = fillInColorProperties(
    gameStats.Climates,
  );
  cultureColorProperties = fillInColorProperties(
    gameStats.Cultures,
  );
  religionColorProperties = fillInColorProperties(
    gameStats.Religions,
  );
  tradeZoneColorProperties = fillInColorProperties(
    gameStats.TradeZones,
  );
  fertilityColorProperties = fillInColorProperties(
    gameStats.Fertility,
  );
  ...
  developmentData = await prepareData(
    "Development.png",
    progressText,
  );
  ...
  let coastPopCount = await findDistribution(
    nationData,
    popData,
    "nation",
    "coastal population",
  ...
  Object.keys(cultureDistribution).forEach((nationKey) => {
    ...
    Object.keys(cultureDistribution[nationKey]).forEach((CultureKey) => {
      ...
      addToTextOutput(
        `= ${(cultureDistribution[nationKey][CultureKey] * 100) / total} ${CultureKey}.Points\n`,
      );
    });
  ...
  Object.keys(religionDistribution).forEach((nationKey) => {
    ...
    Object.keys(religionDistribution[nationKey]).forEach((ReligionKey) => {
      ...
      addToTextOutput(
        `= ${(religionDistribution[nationKey][ReligionKey] * 100) / total} ${ReligionKey}.Points\n`,
      );
    });
  ...
  let nationPopDistribution = await findDistribution(
    nationData,
  ...
  let nationFuturePopDistribution = await findDistribution(
    nationData,
  ...
  let nationFertilityDistribution = await findDistribution(
    nationData,
    fertilityData,
    "nation",
    "fertility",
...
async function findDistribution(
  ...
  outerName,
  innerName,
  colorToOuterNameMapping,
  colorToInnerNameMapping,
  options,
) {
  let ret = {};
...
async function PromptName(color, getDatasetPointFunction, name) {
  ...
  addToTextOutput(
    `= "${color}" ${name}s.${PromptFieldReturnedText}.Color\n`,
  );
...
async function promptMap(imgArray, msg) {
  ...
  PromptedMissingInfoCanvas.getContext("2d").putImageData(
...
function addToTextOutput(text) {
  autoGeneratedCffTextField.value += text;
}
