...
import { getGameStats, GSAddProperty, GSGetProperty, GSSetProperty } from "../stats/gameStats.js";
...
export function PostStatChange(selection, change) {
    ...
    if (/Levies$/.test(selection) ||
        ...
        if (/Elite/.test(selection)) {
            ...
            if (compare > 0) {
                ...
                if (compare > 0)
                    error(`The EliteUnitsCap (${cap}) has been exceeded by ${compare}`);
            }
        }
    ...
    //public debt taken exceed check
        ...
        if (PossibleDebt - change < 0)
            error(`The PublicDebtTaken (${DebtTaken}) has been exceeded by ${change - PossibleDebt}`);
    }
...
export function PostStatCreate(selection, name) {
    if (/Nations$/.test(selection)) {
        ...
        for (const rel in religions) {
            ...
            if (typeof GSGetProperty(religionGroupsSelector) !== 'undefined')
                continue;
        ...
        for (const cul in cultures) {
            ...
            if (typeof GSGetProperty(cultureGroupsSelector) !== 'undefined')
                continue;
        ...
        for (const natName in nations) {
            ...
            if (typeof GSGetProperty(religionGroupSelector) === 'undefined')
                continue;
