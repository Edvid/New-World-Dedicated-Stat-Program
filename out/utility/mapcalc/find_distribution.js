...
export async function findDistribution(outerDataset, innerDataset, outerName, innerName, colorToOuterNameMapping, colorToInnerNameMapping, reportingElements, options) {
    const ret = {};
...
async function PromptName(color, getDatasetPointFunction, name, queryElements) {
    ...
    const retName = await promptMap(dat, queryElements, `The color #${color} did not have a matching ${name}. Which ${name} is it?\n(Give the name it has in stats)`);
    const ret = {
        color: color,
        name: retName,
    };
...
async function promptMap(imgArray, queryElements, msg) {
    let submitted = false;
