...
nationImage.onload = async function () {
    ...
    climateImage.onload = async function () {
        ...
        for (let distanceFromClaim = 0; distanceFromClaim < shipRangeHigh; distanceFromClaim++) {
            let paintColor;
            if (distanceFromClaim <= shipRangeLow)
                paintColor = shipRangeLowColor;
            else if (distanceFromClaim <= shipRangeMid)
            ...
            for (const j of SetRead) {
                ...
                if (distanceFromClaim > shipRangeLow)
                    growFromColours.push(shipRangeMidColor);
                if (distanceFromClaim > shipRangeMid)
                    growFromColours.push(shipRangeHighColor);
        ...
        for (let distanceFromClaim = 0; distanceFromClaim < shipRangeHigh / 2; distanceFromClaim++) {
            ...
            if (distanceFromClaim <= shipRangeLow / 2)
                paintColor = shipRangeLowSmallColor;
            else if (distanceFromClaim <= shipRangeMid / 2)
            ...
            for (const j of SetRead) {
                ...
                if (distanceFromClaim > shipRangeLow / 2)
                    growFromColours.push(shipRangeMidSmallColor);
                if (distanceFromClaim > shipRangeMid / 2)
                    growFromColours.push(shipRangeHighSmallColor);
                growIntoColours = [
                    waterColorArray,
                ];
                if (distanceFromClaim <= shipRangeMid / 2) {
...
function OneNeighbourIsOneOfColors(cols, x, y) {
    for (let Y = -1; Y <= 1; Y++) {
        for (let X = -1; X <= 1; X++) {
            if (isOneOfColorsAtCoord(cols, X + x, Y + y))
                return true;
        }
    }
    return false;
    ...
    for (let Y = -radius; Y < radius; Y++) {
        for (let X = -radius; X < radius; X++) {
            if (X * X + Y * Y > radius * radius)
                continue;
            if (nationClaimPixelAtCoord(X + x, Y + y))
                return true;
        }
    }
    return false;
